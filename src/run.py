...
def _config_WN18():
    ...
    config['iteration'] = tf.constant(200)
    config['dim'] = tf.constant(150)
    config['kns'] = tf.constant(1)
    config['kns_r'] = tf.constant(1)
    config['alpha'] = tf.Variable(0.01, dtype = tf.float64)
    config['beta'] = tf.Variable(0.005, dtype = tf.float64)
...
if __name__ == "__main__":
    ...
    config = _config_WN18()
    # config['hW'] = tf.Variable(0.45, dtype = tf.float64)
    # config['tW'] = tf.Variable(0.45, dtype = tf.float64)
    # config['rW'] = tf.Variable(0.1, dtype = tf.float64)
    # print(config['hW'], config['tW'], config['rW'])
    # train(config)
    
    # config['hW'] = tf.Variable(0.4, dtype = tf.float64)
    # config['tW'] = tf.Variable(0.4, dtype = tf.float64)
    # config['rW'] = tf.Variable(0.2, dtype = tf.float64)
    # print(config['hW'], config['tW'], config['rW'])
    # train(config)

    # config['hW'] = tf.Variable(0.55, dtype = tf.float64)
    # config['tW'] = tf.Variable(0.35, dtype = tf.float64)
    # config['rW'] = tf.Variable(0.1, dtype = tf.float64)
    # print(config['hW'], config['tW'], config['rW'])
    # train(config)

    config['hW'] = tf.Variable(0.2, dtype = tf.float64)
    config['tW'] = tf.Variable(0.6, dtype = tf.float64)
    config['rW'] = tf.Variable(0.1, dtype = tf.float64)
    print(config['hW'], config['tW'], config['rW'])
    train(config)